<?php 
include "config.php";
top_structure('Outward ', 0, 'error', 'Success', 'done');
if(isset($_POST[submit])){
    $total=$d_detail[wallet]+$_POST[amt];
    $outward_total=$d_detail[outward]+$_POST[amt];
    
    $qry="INSERT INTO `io_outward`(`o_id`, `u_id`,`name`, `mobile`, `purpose`, `type`, `amount`,`total_amount`) VALUES (NULL,'$_SESSION[u_id]','$_POST[name]','$_POST[mob]','$_POST[purpose]','$_POST[payment]','$_POST[amt]','$total');";
    
    $qry1="UPDATE `io_user` SET `inward`='$outward_total',`wallet`='$total' WHERE `u_id`='$_SESSION[u_id]';";
   
    if($con->query($qry) === TRUE AND $con->query($qry1) === TRUE){
        echo "<script>location.href='outward.php?n=s';</script>";
    }else{
        echo "<script>location.href='outward.php?n=f';</script>";
    }
